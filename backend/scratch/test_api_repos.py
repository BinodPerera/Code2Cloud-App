...
async def test_get_repos():
    ...
    async with httpx.AsyncClient(timeout=30.0) as http_client:
        ...
        if response.status_code == 200:
            repos = response.json()
            print(f"Total repos returned by exact API call params: {len(repos)}")
            collabs = [r.get("full_name") for r in repos if "BinodPerera" not in r.get("full_name")]
            print(f"Collaborated repos returned: {collabs}")
        else:
